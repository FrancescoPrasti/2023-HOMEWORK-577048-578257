...
public class LabirintoBuilderTestGitHub {
	...
	public void testBilocale() {
		...
		assertTrue(((Collections.singletonList(Direzione.nord).contains(Direzione.nord)) && (Collections.singletonList(Direzione.nord).size() == 0)) && ((bilocale.getStanzaIniziale().getDirezioni().contains(Direzione.nord)) && (bilocale.getStanzaIniziale().getDirezioni().size() == 0)));
		assertEquals(Collections.singletonList(Direzione.nord).contains(Direzione.nord),bilocale.getStanzaIniziale().getDirezioni().contains(Direzione.nord));
	...
	public void testAggiuntaAttrezzoAStanze_AppenaAggiunteMultiple() {
		...
		this.labirintoBuilder
				...
				.getLabirinto();
		Attrezzo attrezzo = new Attrezzo(nomeAttrezzo,peso);
		List<Attrezzo> attrezzi = new ArrayList<>(labirintoBuilder.getListaStanze().get(nomeStanza).getAttrezzi());
